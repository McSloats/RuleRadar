#!/usr/bin/env python3
"""
Daily security detection monitor for:
...
def upload_report(cfg: dict, token: str, filename: str, md_content: str):
    ...
    payload = {
        "message": f"chore: add security monitor report {filename}",
...
def build_markdown(
    ...
    lines = [
        f"# Security Detection Monitor — {timestamp}",
        "",
        "## Summary",
        "",
    ...
    lines += [
        "---",
        "",
        "*Generated by [security-monitor](https://github.com/SigmaHQ/sigma)*",
    ]
...
def send_discord(webhook_url: str, summary: str, filename: str, md_content: str):
    boundary = "SecurityMonitorBoundary47x"
    payload  = json.dumps({"content": summary, "username": "Security Monitor"})
...
def main():
    ...
    print(f"[{now_et.isoformat()}] Running security monitor")
    print(f"  Checking commits since {since_iso}")
    if not YAML_AVAILABLE:
        print("  WARNING: PyYAML not installed — using basic parser (pip install pyyaml)")
    if not SIGMA_BACKEND_AVAILABLE:
        print("  NOTE: pySigma-backend-splunk not installed — Sigma→SPL conversion disabled")
    ...
    rel_count = len(sigma_rels) + len(splunk_rels)

    parts = [f"**Security Detection Monitor — {timestamp}**"]
    parts.append(
